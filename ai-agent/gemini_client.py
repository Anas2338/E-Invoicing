...
Provides rate-limited access to Google Gemini API (free tier) for
efficient decision-making in invoice processing.
"""
import logging
import time
import json
...
class GeminiClient:
    """
    Wrapper for Google Gemini API with rate limiting.

    Implements token bucket algorithm for rate limiting to stay within
    free tier quotas while maximizing throughput.
    """
    ...
    def __init__(self):
        ...
        self.rate_limit_rpm = config.GEMINI_RATE_LIMIT_RPM
        self.tokens = self.rate_limit_rpm  # Start with full bucket
        self.last_refill = time.time()
        self.tokens_per_second = self.rate_limit_rpm / 60.0

        logger.info(f"Gemini API client initialized (model: {config.GEMINI_MODEL}, rate limit: {self.rate_limit_rpm} RPM)")
    ...
    def classify_error(
        ...
        """
        Use Gemini to classify an error as transient or permanent.
