    ...
    def _transform_items_to_fbr_format(self, items: list, uom_mapping: Dict[str, str]) -> list:
        ...
        for item in items:
            ...
            transformed_item = {
                ...
                "rate": rate,
                "uoM": uom_description,
                "quantity": round(float(item.get("quantity", 0)), 4),
                "totalValues": round(float(item.get("total_values", 0)), 2),
                "valueSalesExcludingST": round(float(item.get("value_sales_excluding_st", 0)), 2),
                "fixedNotifiedValueOrRetailPrice": round(float(item.get("fixed_notified_value_or_retail_price", 0)), 2),
                "salesTaxApplicable": round(float(item.get("sales_tax_applicable", 0)), 2),
                "salesTaxWithheldAtSource": round(float(item.get("sales_tax_withheld_at_source", 0)), 2),
                "extraTax": round(float(item.get("extra_tax", 0)), 2),
                "furtherTax": round(float(item.get("further_tax", 0)), 2),
                "sroScheduleNo": item.get("sro_schedule_no", ""),
                "fedPayable": round(float(item.get("fed_payable", 0)), 2),
