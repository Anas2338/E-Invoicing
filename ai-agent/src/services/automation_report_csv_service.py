"""
Automation report CSV generation service.

Generates a line-item-level CSV export over automation invoices, with one row
per invoice item. Invoice-level fields (invoice number, buyer, date, status,
...) are repeated for each item row, so the export is self-contained and can be
pivoted/aggregated in Excel or BI tools.

Mirror of the main backend's ``report_csv_service.py``; the only difference is
the input: these functions take the normalized row dicts produced by
:mod:`automation_report_service` (automation payloads flattened out of the
``invoice_data`` JSON column) rather than ``Invoice`` ORM objects, so the same
columns come out of both services.
...
def generate_report_csv(
    ...
    item columns (edge case: automation invoices always carry at least one
    item, but a malformed upload or a hand-edited payload might not).

    ``item_rate`` and ``uom`` are absent from some upload paths, so those
    cells come out empty for those rows rather than as a misleading zero.
    """
    ...
    for row in rows:
        ...
        invoice_fields = [
            _clean_csv_value(row.get('invoice_number')),
            _clean_csv_value(row.get('fbr_reference_number')),
            _clean_csv_value(row.get('invoice_date')),
            _clean_csv_value(row.get('invoice_type')),
            _clean_csv_value(row.get('buyer_business_name')),
            _clean_csv_value(row.get('buyer_ntn_cnic')),
            _clean_csv_value(row.get('buyer_province')),
            _clean_csv_value(row.get('buyer_address')),
            _clean_csv_value(row.get('seller_business_name')),
            _clean_csv_value(row.get('seller_ntn_cnic')),
            _clean_csv_value(row.get('seller_province')),
            _clean_csv_value(row.get('seller_address')),
            _clean_csv_value(row.get('status')),
            _clean_csv_value(row.get('environment')),
        ]
