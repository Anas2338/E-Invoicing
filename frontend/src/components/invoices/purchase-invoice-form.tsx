...
export function PurchaseInvoiceForm({
  ...
  const [sellerVerificationMessage, setSellerVerificationMessage] = useState<string | null>(null);
  ...
  // Buyer information state (user is buyer in purchase invoice)
  ...
  // Seller registration type (for purchase invoice, we track seller registration)
  const [sellerRegistrationType, setSellerRegistrationType] = useState<'Registered' | 'Unregistered'>('Registered');
  ...
  // Auto-fill buyer information from user profile (user is the buyer in purchase invoice)
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        // Auto-fill buyer information from user profile (user is buyer in purchase invoice)
        if (profile.fbr_seller_ntn) {
          setBuyerNTNCNIC(profile.fbr_seller_ntn);
        }
        if (profile.fbr_business_name) {
          setBuyerBusinessName(profile.fbr_business_name);
        }
        if (profile.fbr_seller_province) {
          setBuyerProvince(profile.fbr_seller_province);
        ...
        if (profile.fbr_seller_address) {
          setBuyerAddress(profile.fbr_seller_address);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      ...
      // Populate buyer information (user is buyer in purchase invoice)
      ...
      setSellerRegistrationType(initialData.seller_registration_type || 'Registered');
  ...
  // Verify seller registration with FBR (in purchase invoice, we verify the seller)
  const verifySellerRegistration = useCallback(async (ntnCnic: string) => {
    ...
    setSellerVerificationMessage(null);
    ...
    try {
      ...
      if (result.success) {
        ...
        setSellerRegistrationType(result.registration_type as 'Registered' | 'Unregistered');

        // Show success message
        if (result.is_registered) {
          if (result.business_name) {
            setSellerVerificationMessage(`✓ Registered: ${result.business_name}`);
          } else {
            setSellerVerificationMessage('✓ Verified as Registered');
          }
        } else {
          setSellerVerificationMessage('✓ Verified as Unregistered');
        }
      } else {
        // Show error but don't block the form
        setSellerVerificationMessage(result.error || 'Could not verify seller registration');
      }
    } catch (error) {
  ...
  const fetchTaxRates = useCallback(async () => {
    ...
    // In purchase invoice, use buyer's province code (user's province)
    if (!transactionTypeId || !invoiceDate || !buyerProvinceCode) {
      setDynamicTaxRates([]);
  ...
  }, [transactionTypeId, invoiceDate, buyerProvinceCode, hasSelectedTransactionType]);
  ...
  // Auto-calculate Further Tax for all items when seller registration type changes
  // In purchase invoice, if SELLER is unregistered, buyer pays 4% further tax
  useEffect(() => {
    setItems(prevItems => {
      return prevItems.map(item => {
        ...
        if (sellerRegistrationType === 'Unregistered') {
          ...
          // Clear Further Tax for Registered sellers and recalculate Total Value
    ...
    setItems(prevItems => {
      ...
      if (field === 'valueSalesExcludingST' && value) {
        ...
        if (valueExclTax > 0 && taxRate >= 0) {
          ...
          // Calculate Further Tax (4%) for Unregistered sellers (in purchase invoice)
          let furtherTax = 0;
          if (sellerRegistrationType === 'Unregistered') {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Validate Further Tax for Unregistered sellers (in purchase invoice)
    if (sellerRegistrationType === 'Unregistered') {
      const itemsWithoutFurtherTax = items.filter(item => !item.furtherTax || item.furtherTax === 0);
      if (itemsWithoutFurtherTax.length > 0) {
        toast.error('Further Tax is required for all items when seller is Unregistered');
        return;
      }
    }
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-6">
        ...
        <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Sale Invoice = Purchase from seller | Debit Note = Adjustment/correction
                  </p>
                </div>
          ...
          {environment === 'SANDBOX' && (
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                Required for sandbox testing. Default: SN001 (Purchase from registered sellers)
              </p>
            </div>
          )}
        </CardContent>
      ...
      {/* Buyer Information (User is the buyer in purchase invoice) */}
      <Card>
        <CardHeader>
          <CardTitle>Buyer Information (Your Information)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="sellerRegistrationType">Registration Type *</Label>
              <Select value={sellerRegistrationType} onValueChange={(val) => setSellerRegistrationType(val as 'Registered' | 'Unregistered')}>
                <SelectTrigger disabled={isVerifyingSeller}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              <Label htmlFor="sellerNTNCNIC">
                Seller NTN/CNIC {sellerRegistrationType === 'Registered' ? '*' : '(Optional)'}
              </Label>
              <div className="relative">
                <Input
                  ...
                  placeholder="Enter NTN or CNIC"
                  required={sellerRegistrationType === 'Registered'}
                  className={isVerifyingSeller ? 'pr-10' : ''}
                />
                {isVerifyingSeller && (
              ...
              {sellerVerificationMessage && (
                <p className={`text-xs mt-1 ${sellerVerificationMessage.startsWith('✓') ? 'text-green-600' : 'text-amber-600'}`}>
                  {sellerVerificationMessage}
                </p>
              )}
            </div>
          ...
          {items.map((item, index) => (
            <div key={index} className="border rounded-lg p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <Label>Further Tax {sellerRegistrationType === 'Unregistered' && '*'}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    ...
                    required={sellerRegistrationType === 'Unregistered'}
                    readOnly={sellerRegistrationType === 'Unregistered'}
                    className={sellerRegistrationType === 'Unregistered' ? 'bg-gray-50 dark:bg-gray-800' : ''}
                  />
