...
export function SummaryCard({ title, count, icon, color, bg, subtitle, className = '' }: SummaryCardProps) {
  return (
    <Card className={`hover:shadow-lg transition-shadow duration-150 flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2 shadow-md ${bg || ''} ${className}`}>
      ...
      <div className="flex-1 text-center">
        <div className="text-lg sm:text-xl font-extrabold text-[#202223] dark:text-white">{count}</div>
