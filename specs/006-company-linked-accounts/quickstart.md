...
7. **Delete scope**: employee deleting the owner's manual invoice → allowed; deleting an automation-posted invoice → also allowed (deletion is company-scoped for every member); owner deletes both fine; a single invoice and a bulk selection of the above behave the same.
