...
class FileManagementService:
    ...
    def delete_invoice(self, invoice_id: str, user_id: str) -> Tuple[bool, str]:
        ...
        if invoice.status in ["transferred", "validated"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot delete transferred or validated invoices. Transferred invoices are permanent for audit purposes. Pause the invoice first if you need to prevent transfer."
            )
